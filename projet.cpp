...
asci_alph=97;
      if(col>9){
	for(j=2;j<10;j++){  plateau[0][j]=asci_num;  asci_num++; }

	for(j=10;j<col;j++){	plateau[0][j]=asci_alph; asci_alph++; }
		 }
	else{
...
int lig,col;
lig=size*2; col=(size*4)-2;
	//clrscr();
for(int i=0;i<lig;i++){
	cout<<"\t \t";
	for(int j=0;j<col;j++){
	 	if(plateau[i][j]=='1' && ( i!=(0 && 1)&&j!=(0 && 1) )){	cout << "\033[0;"<<41<<"m"<<"  "<<plateau[i][j]<<"\033[0m";;}
	 	else if(plateau[i][j]=='0' && (i!=0 && j!=0) ){	cout << "\033[0;"<<42<<"m"<<"  "<<plateau[i][j]<<"\033[0m";;}
	 	else if(plateau[i][j]=='x' && (i!=0 && j!=0) ){	cout << "\033[0;"<<43<<"m"<<"  "<<plateau[i][j]<<"\033[0m";;}
...
bool start:: GameBorder(int lig,int col){
	 if ( (col> size*4-2) || (lig> size*2-1) ){ return false;}
	else if(plateau[lig][col]==' '){ return false;}
	else if(plateau[lig][col]=='*'){ return true;}
	else{return false;}
...
char tmp;
		if(player==1){tmp='1';}
	   else if(player==2){tmp='0';}
	   else if(player==3){tmp='x';}
		
	for(int i=0;i<bridge_array.size();i++){
		int tmp_lig=bridge_array[i]/100;
		int tmp_col=bridge_array[i]%100;

		if(Bridge(tmp_lig,tmp_col,tmp_lig,tmp_col,tmp)){CaseJouer=CaseDeJeux;  break;} 
			else{ save.clear();}						
...
bool start:: Bridge(int a,int b,int c,int d,char g){
	...
	if(Case_De_Trois_Frere(a,b) && (Case_De_Trois_Frere(c,d)) && ((c!=a) || (d!=b)) && (plateau[a][b]==plateau[c][d]) ) { return true;}
		...
		if( (plateau[c-1][d+1]==g) && (Visited_Case(c-1,d+1)==false) ) {
			c--;			
			d++; 
			if(Bridge(a,b,c,d,g)){ return true;} 
			 d--;			
			 c++;		}
		//2
	...
	for(int i=0;i<fork_array.size();i++){
		...
		int tmp_col=fork_array[i]%100;
			...
			if(half_fork('A',tmp_lig,tmp_col,tmp_lig,tmp_col,tmp)){ 
						save.clear(); 
					   for(int i=0;i<fork_array.size();i++){ // for2
						int tmp_lig=fork_array[i]/100;
						int tmp_col=fork_array[i]%100;
...
if( Type == 'A' && Case_De_Quatre_Frere(a,b) && Case_De_Quatre_Frere(c,d) && zone(a,b)!= zone(c,d) && (plateau[a][b]==plateau[c][d])) 					{ A=a; B=b; C=c; D=d; return true;	}

else if(Type =='B' && Case_De_Quatre_Frere(a,b) && Case_De_Quatre_Frere(c,d) && zone(a,b)!= zone(c,d)&& (plateau[a][b]==plateau[c][d]) &&((c==A && d==B) ||(c==C && d==D)) ) 		{ return true;	}
		...
		if( (plateau[c-1][d+1]==g) && (Visited_Case(c-1,d+1)==false) ) {
			c--;			
			d++; 
			if(half_fork(Type,a,b,c,d,g)){ return true;} 
			 d--;			
			 c++;		}
		//2
		else if( (plateau[c-1][d-1]==g) && (Visited_Case(c-1,d-1)==false) ){
...
for(int j=0;j<save.size();j++){
	...
	for(int i=0;i<ring_array.size();i++){ //for1
		...
		int t_col=ring_array[i]%100;	
				 if ( (t_lig!=lig) && (t_col!=col) && ((t_lig>lig+1 || t_lig<lig-1 ) || (t_col>col+2 || t_col<col-2 ))  ){
					...
					//cout<<"t-lig = "<<t_lig<<" t_col = "<<t_col<<endl;
								//show_save();
					if( half_Ring(lig,col,lig,col,t_lig,t_col,tmp)){	//if 2
					//		cout<<"before erase: ";show_save();
						        save.erase(save.begin()+k);
					//		cout<<"After erase: ";show_save();
							     if( half_Ring(t_lig,t_col,t_lig,t_col,lig,col,tmp)){ //if 3
							                     CaseJouer=CaseDeJeux;				  									}// if3
	
								else{ save.insert(save.begin()+k,tt);}
														
														} // if 2
						
							} // end if 1
					if(CaseJouer==CaseDeJeux){break;}
				         else { save.clear();}	
						}//end for1
...
bool start:: half_Ring(int a,int b,int c,int d,int e,int f,char g){
	...
	if(     // ( (c>a+1) || (c<a-1) ) &&
		 //( (d>b+2) || (d<b-2 ) ) &&
		 ( (c==e)  && (d==f) ) && 
		 (plateau[a][b]==plateau[c][d]) ) {   return true; }
		...
		//1		
		 if( (plateau[c-1][d+1]==g) && (Visited_Case(c-1,d+1)==false) ) {
			c--;			
			d++; 	
			if(half_Ring(a,b,c,d,e,f,g)){ return true;} 
			 d--;			
			 c++;		
				}
		//2
...
//##############################################################################
	bool start:: Case_De_Quatre_Frere(int lig,int col){
	 ...
	 if( ( (lig !=1)&&(lig<size)) && (col<size)  ) { return true; }
	 else if( (lig >size) &&  (col<=size-1)  ) { return true; } 								
   	 else if( ((lig !=1)&&(lig<size)) && (col>size*4-2-size) ) { return true;}
   	 else if( (lig >size) && (col>size*4-2-size) ) { return true;}   
  	 else if( (lig ==1)  && (col>size) &&(col<size*4-2-size) ) { return true;}      
  	 else if( (lig ==size*2-1  )  && (col>size) &&(col<size*4-2-size) ) { return true;}   
   	 else { return false;}
	...
	int start:: zone(int lig,int col){
	  	 ...
	  	 if( (lig ==1)  && (col>size) &&(col<size*4-2-size) ) { return 1;}
		 else if( ((lig !=1)&&(lig<size)) && (col>size*4-2-size) ) { return 2;}		
		 else if( (lig >size) && (col>size*4-2-size) ) { return 3;}
		 else if( (lig ==size*2-1  )  && (col>size) &&(col<size*4-2-size) ) { return 4;}
		 else if( (lig >size) &&  (col<=size-1)  ) { return 5; } 						
                 else if( ( (lig !=1)&&(lig<size)) && (col<size)  ) { return 6; }
		// else { return 0;}
	...
	if( ( (lig==1) && ((col==size) || (col==size*4-2-size)) ) || ((lig==size) && ((col==1) || (col==size*4-3)) ) || 
					( (lig==size*2-1) && ( (col==size) || (col==size*4-2-size)) ) )   {return true;}
	else{ return false;}
...
if(trn=="Y" || trn=="y"){ turn=0;}
else if(trn=="N" || trn=="n"){ turn=1;}
	...
	while(CaseJouer!=CaseDeJeux) {
		...
		if(turn % 2 == 0) {
			 ...
			 if(GameBorder(Lig,Col)){
				plateau[Lig][Col]='1'; compteur++;
			//	AfficherPlateau();
				if(CheckWin(1,Lig,Col)==true) {AfficherPlateau();
...
test_mini=0;
			...
			if(CheckWin(2,AImove.x,AImove.y)){AfficherPlateau();
...
cout<<"Minimax"<<endl;
	...
	for(int x = 1; x < size*2; x++) {
		for(int y = 1; y < size*4-2; y++) {
				if(plateau[x][y]!=' '){
						if (Case_De_Trois_Frere(x,y)){bridge_array_minimax.push_back(x*100+y); }	
						else if	(Case_De_Quatre_Frere(x,y)){fork_array_minimax.push_back(x*100+y);}
							}
							}
				}
...
cout<<"Maxsearch"<<endl;
	if(Gameover_minimax()) {//bridge_array_minimax.clear();
				//fork_array_minimax.clear();
					return score();}
	Move bestMove;
...
if(follow==1){ return 10;}
else if(follow==2){ return -10;}	
return 0; // draw
}
...
if (CaseJouer_minimax==CaseDeJeux_minimax){follow=Player;return true;}
Fork_minimax(Player);
if (CaseJouer_minimax==CaseDeJeux_minimax){follow=Player;return true;}
...
cout<<"GaME OVER TEST"<<endl;
	if(CheckWin(1)) { cout<<"GaME OVER TEST Human"<<endl;return true;}
	else if(CheckWin(2)) {cout<<"GaME OVER TEST pc"<<endl;return true;}
...
char tmp;
	...
	for(int i=0;i<bridge_array_minimax.size();i++){
		...
		if(Bridge(tmp_lig,tmp_col,tmp_lig,tmp_col,tmp)){CaseJouer_minimax=CaseDeJeux_minimax;  break;} 
			else{ save.clear();}						
...
void start::  Fork_minimax(int player){
	...
	for(int i=0;i<fork_array_minimax.size();i++){
		...
		int tmp_col=fork_array_minimax[i]%100;
			...
			if(half_fork('A',tmp_lig,tmp_col,tmp_lig,tmp_col,tmp)){ 
						save.clear(); 
					   for(int i=0;i<fork_array_minimax.size();i++){ // for2
						int tmp_lig=fork_array_minimax[i]/100;
						int tmp_col=fork_array_minimax[i]%100;
							if( zone(tmp_lig,tmp_col)!=zone(A,B) &&  zone(tmp_lig,tmp_col)!=zone(C,D))
										{
										if(half_fork('B',tmp_lig,tmp_col,tmp_lig,tmp_col,tmp))
									                             ...
									                             {				  	         										     CaseJouer_minimax=CaseDeJeux_minimax;  break; 
													}
										}
									 } // end for 2
							    	       }
